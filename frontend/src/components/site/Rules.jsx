...
export default function Rules() {
  return (
    <section id="rules" data-testid={CARHIRE.rulesSection} className="py-20 md:py-28 bg-[#1A1A1A] text-[#F9F6F0] relative overflow-hidden">
      <div className="absolute top-20 -left-20 w-96 h-96 bg-[#B65F33]/15 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 -right-32 w-[28rem] h-[28rem] bg-[#D4AF37]/10 rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="max-w-2xl mb-12">
          <div className="text-xs uppercase tracking-[0.22em] text-[#D4AF37] font-semibold mb-3">
            Rental Terms
          </div>
          <h2 className="font-display text-4xl md:text-5xl tracking-tight">
            Clear rules for a smooth ride.
          </h2>
          <p className="mt-4 text-[#F9F6F0]/70">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {RULES.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className="rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 p-6 hover:border-[#D4AF37]/40 hover:bg-white/[0.07] transition-all duration-300"
            >
              <div className="h-11 w-11 rounded-xl grid place-items-center bg-[#D4AF37]/15 text-[#D4AF37] mb-4">
                <Icon className="h-5 w-5" />
              </div>
              <h3 className="font-display text-xl" dangerouslySetInnerHTML={{ __html: title }} />
              <p className="text-sm mt-2 text-[#F9F6F0]/70">{desc}</p>
            </div>
          ))}
        </div>
      </div>
